...
describe('Employee Service Unit Tests', () => {
    ...
    before(() => {
      sinon.stub(Employee, 'find').callsFake((obj) => {
        const db = [{
          ...
          firstName: 'John',
          lastName: 'Smith',
          phoneNumber: '41414565',
          links: [],
          setupHyperLinks: () => null,
        }];
        return db.filter(employee => employee.firstName === obj.firstName)[0];
      });
    });
  ...
  describe('findEmployeeById', () => {
    ...
    it('should return a promise that eventually resolved to an object', async () => {
  ...
  describe('populate', () => {
    ...
    it('should throw if select arguement is not a string', async () => {
      const obj = { id: '123' };
      const path = 'test';
      const select = ['test'];
      await expect(populate(obj, path, select))
  ...
  describe('copyObjectAndAddLastChanged', () => {
    ...
    it('should add a lastChanged property to returned object', () => {
      const obj = {
        _id: '123',
        links: [],
      };
